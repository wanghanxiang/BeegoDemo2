...
func insertArticle(article Article) (int64, error) {
	return util.ModifyDB("insert into article(title,tags,short,content,author,createtime) values(?,?,?,?,?,?)",
...
func QueryArticleRowNum() int {
	row := util.QueryRowDB("select count(id) from article")
	num := 0
	row.Scan(&num)
	return num
}
...
func QueryArticleWithId(id int) Article {
	row := util.QueryRowDB("select id,title,tags,short,content,author,createtime from article where id=" + strconv.Itoa(id))
	title := ""
	tags := ""
	short := ""
	content := ""
